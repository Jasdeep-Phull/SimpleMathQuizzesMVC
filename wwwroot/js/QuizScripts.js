 ...
 */
function createQuiz(submitUrl, successUrl, numberOfQuestions) {
    ...
    if (confirm("Are you sure you want to submit your answers?")) {
        ...
        console.log("Questions: ".concat(viewQuestions));
        console.log("Answers: ".concat(viewAnswers));
        // data object
        var createData = { "Questions": viewQuestions, "UserAnswers": viewAnswers };
        console.log("createData.Questions: ".concat(createData.Questions));
        console.log("createData.UserAnswers: ".concat(createData.UserAnswers));
        // request body
        var sendData = JSON.stringify(createData);
        console.log("sendData: ".concat(sendData));
        // get the CSRF token on the page
        var verificationToken = $('input[name="__RequestVerificationToken"]').val().toString();
        console.log("v token: ".concat(verificationToken));
        // ajax call
        $.ajax({
            type: "POST",
            url: submitUrl,
            ...
            success: function (response) {
                ...
                if (response === null) {
                    ...
                    console.log("response message: ".concat(response.message));
            ...
            error: function (jqXHR, status, error) {
                ...
                if (jqXHR === null) {
                    ...
                    if (jqXHR.responseJSON === null) {
                        ...
                        console.log("Error ".concat(jqXHR.status)); // <----
                        console.log("Error message: ".concat(jqXHR.responseJSON.message));
                        alert("".concat(jqXHR.responseJSON.message, "\nHTTP: ").concat(jqXHR.status));
                    }
                }
            }
        });
 ...
 */
function updateQuiz(submitUrl, successUrl, numberOfQuestions, quizId) {
    console.log("submit called");
    if (confirm("Are you sure you want to submit your answers?")) {
        ...
        console.log("updateData.Id: ".concat(updateData.Id));
        console.log("updateData.UserAnswers: ".concat(updateData.UserAnswers));
        // request body
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response === null) {
                    console.log("json response was null");
                    alert("Successfully edited quiz (ID: ".concat(quizId, ")"));
                }
                else {
                    console.log("response message: ".concat(response.message));
                    alert(response.message);
...
function getQuestions(numberOfQuestions) {
    ...
    for (var i = 0; i < numberOfQuestions; i++) {
        var questionId = "#question".concat(i.toString());
        console.log("qId: ".concat(questionId));
        var question = $(questionId).text();
        console.log("  question: ".concat(question));
        questions.push(question);
    }
    console.log("questions: ".concat(questions));
    return questions;
}
/**
 ...
 */
function getAnswers(numberOfQuestions) {
    console.log("getAnswers called");
    var userAnswers = [];
    for (var i = 0; i < numberOfQuestions; i++) {
        var answerId = "#answer".concat(i.toString());
        console.log("aId: ".concat(answerId));
        var stringUserAnswer = $(answerId).val().toString();
        console.log("  stringAnswer: ".concat(stringUserAnswer));
        // check if the answer is valid, return null if the answer is invalid
        var userAnswer = checkAnswer(stringUserAnswer);
        console.log("    userAnswer: ".concat(userAnswer));
        userAnswers.push(userAnswer);
    }
    console.log("userAnswers: ".concat(userAnswers));
    return userAnswers;
}
/**
...
function checkAnswer(userAnswer) {
    ...
    var tempAnswer = userAnswer.trim();
    console.log("3answer: ".concat(userAnswer, ", tempAnswer: ").concat(tempAnswer));
    // check against regex
    if (/^(-)?(0|([1-9](\d)*))$/.test(tempAnswer)) {
        var intAnswer = parseInt(tempAnswer, 10);
        console.log("3tempAnswer: ".concat(tempAnswer, ", intAnswer: ").concat(intAnswer));
        // check if parsed answer is NaN
        if (isNaN(intAnswer) === true) {
            // log error
            console.log("3answer: ".concat(userAnswer, " is invalid, was Nan after parseInt()"));
            return null;
        }
        else {
            console.log("3answer: ".concat(userAnswer, " is valid"));
            return intAnswer;
        }
    }
    else {
        console.log("3answer: ".concat(userAnswer, ", \" is invalid"));
        return null;
    }
}
//# sourceMappingURL=QuizScripts.js.map